...
public class ThreadedCursor {
    protected Map<Node, List<Node>> replicas;
    ...
    public ThreadedCursor(Map<Node, List<Node>> replicas, 
    ...
    protected class Dispatcher extends Thread {
        ...
        public void run() {
            ...
            for (Map.Entry<Node, List<Node>> entry :
                    replicas.entrySet()) {
                // attempt primary replica
                Node primaryNode = entry.getKey();
                try {
                    this.submitQuery(primaryNode.getId(),
                        primaryNode.getIpAddress(),
                        primaryNode.getPort());

                    continue; // success
                } catch (Exception e) {
                    System.err.println("failed to contact primary node "
                        + primaryNode.getIpAddress()
                        + ":" + primaryNode.getPort()
                        + " - " + e.getMessage());
                }
                ...
                for (Node secondaryNode : entry.getValue()) {
                    try {
                        this.submitQuery(secondaryNode.getId(),
                            secondaryNode.getIpAddress(),
                            secondaryNode.getPort());

                        break; // success
                    } catch (Exception e) {
                        System.err.println("failed to contact secondary node "
                            + secondaryNode.getIpAddress()
                            + ":" + secondaryNode.getPort()
                            + " - " + e.getMessage());
                    }
